...
\begin{center}
  \IfFileExists{images/check_ops.png}{\includegraphics[width=0.85\textwidth]{images/check_ops.png}}{[截图占位：check\_ops]}
\end{center}

\subsection{正确性测试（\texttt{./btest}）}

\begin{center}
  \IfFileExists{images/btest.png}{\includegraphics[width=0.85\textwidth]{images/btest.png}}{[截图占位：btest]}
\end{center}
...
\section{实验感受（可选）}
本实验的乐趣在于把``显然''的运算翻译成受限运算符的组合：溢出判定、
向偶舍入与浮点的分段处理都要求对补码和 IEEE 754 的编码本身有精确理解，
而不只是会用。操作数上限（如 bitReverse 的 34 ops）迫使人去找结构性的
省操作技巧（掩码递推），很有意思。
